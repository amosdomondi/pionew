			...
			case 'product':
				switch($table){
					...
					case 'options':
						...
						$query = 'SELECT pr.product_related_id, p.product_code';
						...
						$database->setQuery($query);
						$rows = $database->loadObjectList('product_related_id');
						$types[$table]->type = 'text';
						$key_id = $table .'_id';
						foreach($elements as $element){
							foreach($element as $key=>$related){
								if($key == $table){
									foreach($related as $product){
										$product->$table = '';
										if(isset($rows[$product->$key_id]))
											$product->$table = $rows[$product->$key_id]->product_code;
									}
								}
							}
						}
			...
			if($key === $column && isset($type->type)){
				switch($type->type){
					...
					case 'options' :
						if($export) {
							$subColumn = $type->type;
							$square = $element->$column->$subColumn;
						} else {
							$square = $element->$column->id.' : '.$element->$column->name;
						}
						break;
