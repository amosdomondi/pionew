...
class hikashopCurrencyClass extends hikashopClass{
	...
	function calculateProductPriceForQuantity(&$product) {
		...
		if(function_exists('hikashop_product_price_for_quantity_in_cart')) {
			...
			$this->quantityPricesForProduct($product, @$product->cart_product_quantity, $product->cart_product_total_quantity);
		}
	...
	function quantityPrices(&$prices, $quantity, $total_quantity) {
		...
		return $this->quantityPricesForProduct($product, $quantity, $total_quantity);
	}

	function quantityPricesForProduct(&$product, $quantity, $total_quantity) {
		...
		$this->quantityPrice($unitPrice, $quantity, $product);
		$prices = array($unitPrice);
	}

	function quantityPrice(&$price, $quantity, $product = null) {
		if($quantity <= 0)
			return;
		...
		$coef = 1;
		if(!empty($product) && !empty(($product->option_parent_product))) {
			$config = hikashop_config();
			$group = $config->get('group_options', 0);
			if($group) {
				$coef = $product->cart_product_quantity;
			}
		}
		...
		foreach($keys as $key) {
			if(!isset($price->$key))
				continue;
			$original = $price->$key;
			$price->unit_price->$key = (float)$this->round($original, $rounding);
			$price->$key = $this->round($price->unit_price->$key * (int)$quantity, $rounding);
			$price->unit_price->$key = (float)$this->round($original*$coef, $rounding);
		}
