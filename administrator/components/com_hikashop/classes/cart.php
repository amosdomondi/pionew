...
class hikashopCartClass extends hikashopClass {
	...
	public function save(&$element) {
		...
		if(!$status) {
			$this->addMessage($element, array('msg' => JText::_('CART_COULD_NOT_BE_SAVED_TO_THE_DATABASE'), 'product_id' => 0, 'type' => 'error'));
			return $status;
		}
	...
	public function &getFullCart($cart_id = 0, $options = array()) {
		...
		foreach($cart->products as &$product) {
			...
			if(!empty($product->cart_product_option_parent_id) && isset($cart->products[ $product->cart_product_option_parent_id ]))
				$product->option_parent_product = $cart->products[ $product->cart_product_option_parent_id ];
