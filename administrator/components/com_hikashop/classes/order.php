...
class hikashopOrderClass extends hikashopClass {
	...
	public function createFromCart($cart_id, $options = array()) {
		...
		foreach($cart->full_products as &$product) {
			...
			if(!empty($product->discount)) {
				...
				$orderProduct->order_product_price_before_discount = $orderProduct->discount->price_value_without_discount;
			...
			if(isset($productData['many']) && $productData['many'] == true) {
				...
				foreach($productData as $key => $value) {
					...
					if(is_array($value))
						$value = implode(',', $value);
					$product->$key = $safeHtmlFilter->clean($value, 'string');
					if(in_array($key, array('order_product_price', 'order_product_tax'))) {
						$product->$key = (float)hikashop_toFloat($product->$key);
					}
				}
	...
	public function loadFullOrder($order_id, $additionalData = false, $checkUser = true) {
		...
		JPluginHelper::importPlugin('hikashop');
		$app = JFactory::getApplication();
		$app->triggerEvent( 'onBeforeLoadFullOrder', array( &$order, &$additionalData, &$checkUser) );
