...
class HikashopBotInc {
    public function is_bot($sistema=null){
        if(is_null($sistema)) {
            $sistema = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
        }
        if(empty($sistema)) {
            return false; // no user agent, so not a bot
        }
        $bots = array(
            'Googlebot'
            , 'Baiduspider'
