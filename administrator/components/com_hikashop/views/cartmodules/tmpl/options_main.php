...
$drawer_style = (empty($this->element['small_cart']) || (int)$this->element['small_cart'] != 4) ? 'display: none;' : '';
$openable_style = (!empty($this->element['small_cart']) && (int)$this->element['small_cart'] > 1) ? '' : 'display: none;';
?>
<div class="hkc-xl-6 hkc-md-6 hikashop_module_subblock hikashop_module_edit_general_part1">
	<div class="hikashop_module_subblock_content">
		...
		<dl class="hika_options hikashop_mini_cart">
			...
			<dd class="hikashop_option_value"><?php
				$values = array(
					...
					4 => JHTML::_('select.option', 4, JText::_('HIKA_CART_DRAWER')),
				);
		...
		<dl class="hika_options"  style="<?php echo $drawer_style; ?>" data-part="drawer_position">
			<dt class="hikashop_option_name">
				<label><?php echo hikashop_hktooltip(JText::sprintf('DRAWER_POSITION_TOOLTIP', $this->type), '', JText::_('DRAWER_POSITION'), '', 0); ?></label>
			</dt>
			<dd class="hikashop_option_value">
				<?php
				$values = array(
					0 => JHTML::_('select.option', 'left', JText::_('HIKA_LEFT')),
					1 => JHTML::_('select.option', 'right', JText::_('HIKA_RIGHT')),
				);
				echo JHTML::_('hikaselect.radiolist', $values, $this->name.'[drawer_position]', '', 'value', 'text', @$this->element['drawer_position']);
				?>
			</dd>
		</dl>
		<dl class="hika_options" style="<?php echo $openable_style; ?>" data-part="auto_open">
			<dt class="hikashop_option_name">
				<label><?php echo JText::_('AUTO_OPEN_ON_ADD_TO_CART'); ?></label>
			</dt>
			<dd class="hikashop_option_value"><?php
				echo JHTML::_('hikaselect.booleanlist', $this->name.'[auto_open]', '', @$this->element['auto_open']);
			?></dd>
		</dl>
<?php
...
window.hikashop.ready(function(){
	...
	hkjQuery("[data-control=\'mini_cart\']").change(function(){
		var smallCart = hkjQuery(this).val();
		if(smallCart == "0")
		...
		if(smallCart != "4")
			hkjQuery("[data-part=\'drawer_position\']").hide();
		else
			hkjQuery("[data-part=\'drawer_position\']").show();

		if(smallCart != "0" && smallCart != "1")
			hkjQuery("[data-part=\'auto_open\']").show();
		else
			hkjQuery("[data-part=\'auto_open\']").hide();
	});
