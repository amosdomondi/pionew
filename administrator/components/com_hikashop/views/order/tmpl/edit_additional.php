	...
	<table class="hikashop_order_additional_table adminlist table table-striped">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<input type="text" id="hikashop_order_discount_price_input" name="hikashop_order_discount_price_without_tax_input" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'discount');" value="<?php echo (@$this->order->order_discount_price-@$this->order->order_discount_tax); ?>" />
				</td>
				<td>
				...
				<td>
					<input type="text" id="hikashop_order_discount_price_with_tax_input" name="data[order][order_discount_price]" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'discount');" value="<?php echo @$this->order->order_discount_price; ?>" />
				</td>
			</tr>
			<tr>
				<td class="additional_row_title">
				...
				<td>
					<input type="text" id="hikashop_order_shipping_price_input" name="hikashop_order_shipping_price_without_tax_input" value="<?php echo ($this->order->order_shipping_price-@$this->order->order_shipping_tax); ?>" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'shipping');" />
				</td>
				<td>
				...
				<td>
					<input type="text" id="hikashop_order_shipping_price_with_tax_input" name="data[order][order_shipping_price]" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'shipping');" value="<?php echo @$this->order->order_shipping_price; ?>" />
				</td>
			</tr>
			<tr>
				<td class="additional_row_title">
				...
				<td>
					<input type="text" id="hikashop_order_payment_price_input" name="hikashop_order_payment_price_without_tax_input" value="<?php echo ($this->order->order_payment_price-@$this->order->order_payment_tax); ?>" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'payment');" />
				</td>
				<td>
				...
				<td>
					<input type="text" id="hikashop_order_payment_price_with_tax_input" name="data[order][order_payment_price]" onkeyup="window.orderMgr.updateAdditionalTaxes(this, 'payment');" value="<?php echo @$this->order->order_payment_price; ?>" />
				</td>
			</tr>
<?php
	if(!empty($this->order->additional)) {
...
window.orderMgr.updateAdditionalTaxes = function(el, type) {
	...
	var conversion = 0;
	var price = priceInput.value.replace(",", ".");
	var target = priceInput;
	if(el.id == priceWithTaxInput.id) {
		conversion = 1;
		price = priceWithTaxInput.value.replace(",", ".");
	} else {
	...
	if(taxRateSelect.value == '-1') {
		...
		window.Oby.xRequest(
			'index.php?option=com_hikashop&tmpl=component&ctrl=product&nofloat=1&task=getprice&price='+price+'&rate_namekey='+encodeURIComponent(taxRateSelect.value)+'&conversion='+conversion,
			{ mode: 'GET'},
			function(result) {
