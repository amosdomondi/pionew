...
?>
		...
		<dd class="hikashop_order_product_price">
			<input type="text" id="hikashop_order_product_price_input" onkeyup="window.orderMgr.updateTaxes(this);" name="data[order][product][order_product_price]" value="<?php echo @$this->orderProduct->order_product_price; ?>" />
		</dd>
...
window.orderMgr.updateTaxes = function(el) {
	...
	if(taxRateSelect.value == '-1') {
		...
		window.Oby.xRequest(
			'index.php?option=com_hikashop&tmpl=component&ctrl=product&task=getprice&nofloat=1&price='+price+'&rate_namekey='+encodeURIComponent(taxRateSelect.value)+'&conversion='+conversion,
			{ mode: 'GET'},
			function(result) {
