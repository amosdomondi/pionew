...
class OrderViewOrder extends hikashopView{
	...
	public function listing() {
		...
		$pageInfo->filter->filter_order_type = hikaInput::get()->getCmd('order_type', 'sale');
		if(empty($pageInfo->filter->filter_order_type))
			$pageInfo->filter->filter_order_type = 'sale';

		$database = JFactory::getDBO();
		$tables = array();
		$filters = array(
			'b.order_type='. $database->quote($pageInfo->filter->filter_order_type)
		);
	...
	function export() {
		...
		if(empty($ids)) {
			...
			$order_type = hikaInput::get()->getCmd('order_type', 'sale');
			if(empty($order_type))
				$order_type = 'sale';
			$filters['order_type'] = 'hk_order.order_type = '. $database->Quote($order_type);
