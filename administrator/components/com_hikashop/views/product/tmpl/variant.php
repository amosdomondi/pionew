	...
	if(!empty($this->product->characteristics)) {
		foreach($this->product->characteristics as $characteristic) { ?>
			...
			<dd class="hikashop_product_characteristic"><?php
				if($edit_variant) {
					$nameboxVariantType = hikashop_get('type.namebox');
					echo $nameboxVariantType->display(
						'data[variant][characteristic]['.$characteristic->characteristic_id.']',
						(int)@$characteristic->default_id,
...
if(hikashop_acl('product/edit/warehouse')) { ?>
	<dt class="hikashop_product_warehouse"><label for="data_variant_product_warehouse_id_text"><?php echo JText::_('WAREHOUSE'); ?></label></dt>
	<dd class="hikashop_product_warehouse"><?php
		echo $this->nameboxType->display(
			'data[variant][product_warehouse_id]',
