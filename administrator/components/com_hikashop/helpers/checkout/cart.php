...
class hikashopCheckoutCartHelper extends hikashopCheckoutHelperInterface {
	...
	public function getParams() {
		...
		$this->params['show_discount']['inherit'] = $default_params['show_discount'];
		$this->params['show_discount']['values'] = array(
			JHTML::_('select.option', 0, JText::_('HIKASHOP_NO')),
			JHTML::_('select.option', 1, JText::_('DISPLAY_DISCOUNT_AMOUNT')),
			JHTML::_('select.option', 2, JText::_('DISPLAY_PRICE_BEFORE_DISCOUNT')),
			JHTML::_('select.option', 3, JText::_('HIKA_INHERIT') . ' (' . JText::_($default_params['show_discount']) . ')'),
		);
	...
	public function display(&$view, &$params) {
		...
		if(!isset($params['show_discount'])) {
			$params['show_discount'] = 0;
		}elseif((int)$params['show_discount'] === 3) {
			$defaultParams = $view->config->get('default_params');
			$params['show_discount'] = (int)$defaultParams['show_discount'];
		}
