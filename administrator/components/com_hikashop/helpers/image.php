...
class hikashopImageHelper {
	...
	public function getDefaultImage() {
		...
		if(!empty($path) && is_file($this->main_uploadFolder . $path))
	...
	public function checkSize(&$width, &$height, &$fileObj) {
		...
		if(!empty($fileObj->file_path) && is_file($this->main_uploadFolder . $fileObj->file_path)) {
	...
	function getThumbnail($filename, $size = null, $options = array(), $relativePath = true, $cachePath = null) {
		...
		if($extension == 'svg') {
			...
			if($cachePath !== false && is_file($cachePath . $destFolder . DS . $filename)) {
				$ret->success = true;
				...
				if($this->needWebp($extension)) {
					if($extension == 'webp') {
						...
						if(!is_file($cachePath . $destFolder . DS . $webpfile)) {
	...
	public function display($path, $addpopup = true, $title = '', $options = '', $optionslink = '', $width = 0, $height = 0, $alt = '') {
		...
		$html = '';
		if(!is_file($this->uploadFolder . $path)) {
			$path = $config->get('default_image');
			if($path == 'barcode.png') {
...
';
	...
	function displayThumbnail($path, $title = '', $reduceSize = false, $options = '', $module = false, $alt = '') {
		...
		if($this->thumbnail){
			...
			if(!is_file($this->uploadFolder.$folder.DS.$path)){
				if($module){
