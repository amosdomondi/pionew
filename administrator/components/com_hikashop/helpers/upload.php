...
class hikashopUploadHelper {
	...
	public function process($options = null) {
		...
		if(empty($slice) || $slice['index'] == 0) {
			...
			if(is_file($this->options['upload_dir'] . $this->options['sub_folder'] . $file_path)) {
			...
			if(substr($file_path, -5) != '.part' || !is_file($this->options['upload_dir'] . $this->options['sub_folder'] . $file_path)) {
