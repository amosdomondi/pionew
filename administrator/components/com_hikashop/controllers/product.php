...
class ProductController extends hikashopController {
		...
		if(isset($file->file_path)) {
			...
			if(!$isLink && !$isVirtual) {
				...
				if($firstChar == '/' || preg_match('#:[\/\\\]{1}#', $file->file_path)) {
					...
					if(!is_file($clean_filename) && (JPATH_ROOT == '' || !is_file(JPATH_ROOT . DS . $clean_filename))) {
	...
	public function getprice() {
		...
		$nofloat = hikaInput::get()->getInt('nofloat');
		...
		if(count($main_tax_zone)&&!empty($tax_id)&&!empty($price)&&!empty($main_tax_zone)){
			...
			$newprice = $currencyClass->$function($price,array_shift($main_tax_zone),$tax_id,5, $nofloat);
		}
		if($currency){
