...
class plgHikashopshippingFedEx2 extends hikashopShippingPlugin
{
	...
	protected function _fedex_request_rates ($method_params, $address, $order) {
		...
		if(empty($address->shipping_address->address_country->zone_code_2)) {
			if(empty($address->shipping_address->address_country))
				$address->shipping_address->address_country =  new stdClass();
			$address->shipping_address->address_country->zone_code_2 = $method_params->shipping_params->country_code;
		}
		$array_address = array(
			'address' =>
				(new Address)
		...
		try {
			...
			if (isset($auth_result->access_token)) {
				...
				if(!empty($method_params->shipping_params->debug)) {
					hikashop_writeToLog('FedEx 2 : Impossible to receive rates from FedEx for request :');
			...
			if(isset($response['errors']) && is_array($response['errors'])) {
				foreach ($response['errors'] as $error) {
					$this->error_messages['fedex2_error'] = 'FedEx 2 : An error occurred. '. $error['message'] .'<br/>Error code :'. $error['code'];
				}
			} else {
				hikashop_writeToLog('FedEx 2 : An error occurred. '. $e->getMessage());
