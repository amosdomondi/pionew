...
";
	...
	function _uploadFiles($name, &$element){
		...
		if(!is_file($folder_path.'b'.DS.'.htaccess')){
...
';
		...
		if($rights[1]!='w' && !(is_file(JPATH_ROOT.DS.'atos.php'))){
	...
	function _copyLogos($safe_mode, &$element) {
		...
		$ok = true;
		foreach($this->cards as $card){
			if(is_file(HIKASHOP_MEDIA.DS.'images'.DS.'payment'.DS.$card.'.gif')){
				if(!(is_file($destFolder.$card.'.gif'))){
	...
	function _checkOnSafeMode($element){
		...
		if(is_dir($param_path)){
			if(!is_file($param_path.'pathfile')){
				$app->enqueueMessage( sprintf($message['file_x_not_exists'], 'pathfile', $param_path.'pathfile'), 'error');
				$fail=true;
			}
			if(!is_file($param_path.'parmcom.'.$element->payment_params->merchant_id)){
			...
			if(!is_file($binaries_path.'certif.'.$element->payment_params->merchant_country.'.'.$element->payment_params->merchant_id)){
		...
		if(!is_file(JPATH_ROOT.DS.'atos.php')){
			$app->enqueueMessage( $message['autoresponse_cannot_be_created'], 'error');
			$fail=true;
		}
	...
	function _checkFiles(&$element){
		...
		if($os=='win'){
			if(!is_file($path.'b'.DS.'request.exe')){
			...
			if(!is_file($path.'b'.DS.'response.exe')){
		...
		if(!empty($element->payment_params->merchant_country))
			$element->payment_params->merchant_country=strtolower($element->payment_params->merchant_country);
		if(!is_file($path.'b'.DS.'ct.'.$element->payment_params->merchant_country.'.'.$element->payment_params->merchant_id)){
			if(!empty($element->payment_params->merchant_country) && !empty($element->payment_params->merchant_id)){
