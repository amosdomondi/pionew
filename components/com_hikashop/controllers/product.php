...
class productController extends hikashopController {
	...
	public function send_email() {
		...
		if($status) {
			...
			if(!empty($product->product_id)) {
				...
				if($redirect) {
					$app->redirect(hikashop_contentLink('product&task=show&cid='.$product->product_id.'&name='.$product->alias.'&contact_request_sent=1'.$url_itemid, $product, false, true));
				} else {
					$app->enqueueMessage(JText::sprintf('CLICK_HERE_TO_GO_BACK_TO_PRODUCT',hikashop_contentLink('product&task=show&cid='.$product->product_id.'&name='.$product->alias.'&contact_request_sent=1'.$url_itemid, $product)));
				}
			}
		}
	...
	public function updatecart() {
		...
		$botInc = hikashop_get('inc.bot');
		if($botInc->is_bot()) {
			throw new Exception(JText::_('BOT_DETECTED_OPERATION_NOT_ALLOWED'), 404);
			echo hikashop_display(JText::_('BOT_DETECTED_OPERATION_NOT_ALLOWED'), 'error');
			return;
		}
