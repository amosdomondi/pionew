...
class orderController extends hikashopController {
	...
	public function pay() {
		...
		if(!hikashop_level(1)) {
			$app->enqueueMessage('Payment of orders not paid immediately is not available on this website. Please contact the website administrator if you have any question.', 'error');
			return false;
		}
		if(!$config->get('allow_payment_button', 1)) {
			$app->enqueueMessage('Payment of orders not paid immediately is not allowed on this website. Please contact the website administrator if you have any question.', 'error');
			return false;
		}
