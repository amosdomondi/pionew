...
class OrderViewOrder extends hikashopView {
	...
	public function pay() {
		...
		hikashop_get('helper.checkout');
		$checkoutHelper = hikashopCheckoutHelper::get();
		$this->payment_selector = 2;
		foreach($checkoutHelper->checkout_workflow['steps'] as $step) {
			if(!empty($step['content'])) {
				foreach($step['content'] as $content) {
					if($content['task'] == 'payment' && empty($content['params']['read_only']) && isset($content['params']['payment_selector'])) {
						$this->payment_selector = $content['params']['payment_selector'];
					}
				}
			}
		}
